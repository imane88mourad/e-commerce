...
export default function QuotesPage() {
  ...
  const load = useCallback(() => {
    ...
    adminQuotesApi.list(params)
      ...
      .catch((err) => { if (active) setError(err.message || 'Erreur chargement devis'); })
    ...
    <div>
      ...
      <Card>
        <div className="px-5 pt-5">
          <Toolbar value={search} onSearch={setSearch} searchPlaceholder="Rechercher (client, email)…">
        ...
        {loading ? (
          ...
          <EmptyState
            icon={<Icon name="quotes" size={28} />}
            title="Aucun devis"
            description="Les devis seront créés depuis le back-office."
          />
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="border-b border-[color:var(--admin-border)]">
                <tr>
                  <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-[color:var(--admin-muted)]">Devis</th>
                ...
                {filteredQuotes.map((q) => (
                  ...
                  >
                    <td className="px-4 py-3">
                      <span className="font-semibold text-[color:var(--admin-accent)]">#{q.id}</span>
                    </td>
                    <td className="px-4 py-3">
                      <div className="font-medium text-[color:var(--admin-text)]">{q.client_name || '—'}</div>
                      <div className="text-xs text-[color:var(--admin-muted)]">{q.client_email || ''}</div>
                    </td>
                    ...
                    <td className="px-4 py-3">
                      <Badge tone={statusTone[q.status] || 'gray'} dot>
                        {QUOTE_STATUSES.find((s) => s.value === q.status)?.label || q.status}
                      </Badge>
                    </td>
                    <td className="px-4 py-3">
                      <div className="flex items-center justify-end">
                        <button title="Voir" onClick={(e) => { e.stopPropagation(); router.push(`/admin/quotes/${q.id}`); }} className="rounded-lg p-2 text-[color:var(--admin-muted)] transition hover:bg-[color:var(--admin-accent-soft)] hover:text-[color:var(--admin-accent)]">
