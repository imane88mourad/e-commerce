        ...
        <>
            ...
            <div className="flex flex-col justify-between px-6 md:px-16 lg:px-32 py-6 min-h-screen">
                <div className="space-y-5">
                    <h2 className="text-lg font-medium mt-6">{t('account.myOrders')}</h2>
                    {loading ? (
                        <Loading />
                    ) : error ? (
                        ...
                        <div className="max-w-5xl border-t border-gray-300 text-sm">
                            {orders.map((order, index) => (
                                <div key={order.id || index} className={`flex flex-col md:flex-row gap-5 justify-between p-5 border-b border-gray-300 ${isRTL ? 'md:flex-row-reverse' : ''}`}>
                                    <div className={`flex-1 flex gap-5 max-w-80 ${isRTL ? 'flex-row-reverse' : ''}`}>
                                        <Image
                                        ...
                                        <p className="flex flex-col gap-3">
                                            ...
                                            <span>{t('cart.items')} : {order.items ? order.items.length : 0}</span>
                                        </p>
                                    </div>
                                    <div>
                                        <p>
                                            {order.guest_full_name ? (
                                                ...
                                                <span className="font-medium">{t('account.guestOrder')}</span>
                                            )}
                                        </p>
                                    </div>
                                    ...
                                    <div>
                                        <p className="flex flex-col">
                                            <span>{t('account.method')} : {order.payment_method === 'cod' ? 'COD' : order.payment_method}</span>
                                            <span>{t('account.date')} : {new Date(order.date).toLocaleDateString()}</span>
                                            <span>{t('account.status')} : <span className="capitalize">{order.status}</span></span>
                                        </p>
                                    </div>
                                </div>
                            ))}
